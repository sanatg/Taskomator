...
def start_torghost():
    ...
    print(bcolors.GREEN + '[done]' + bcolors.ENDC)
    print(bcolors.YELLOW + 'DEVICE GHOSTED')
...
def stop_torghost():
    logo()
    print(bcolors.RED + t() + 'STOPPING Ghoster' + bcolors.ENDC)
    ...
    print(bcolors.GREEN + '[done]' + bcolors.ENDC)
    print(bcolors.YELLOW + 'DEVICE IS NOW NOT GHOSTED')
    print(t() + ' Fetching current IP...')
    time.sleep(3)
    print(t() + ' CURRENT IP : ' + bcolors.GREEN + ip() + bcolors.ENDC)
...
def check_update():
    ...
    if version.parse(newversion) > version.parse(VERSION):
        ...
        print(t() + ' Your current Ghoster version : ' + bcolors.GREEN + VERSION + bcolors.ENDC)
        print(t() + ' Latest Ghoster version available : ' + bcolors.GREEN + newversion + bcolors.ENDC)
        yes = {'yes', 'y', 'ye', ''}
        no = {'no', 'n'}
